...
const SKIP_RE   = /balance\s+(carried|brought|from)\s+|brought\s+forward|opening\s+balance|closing\s+balance|account\s+summary|previous\s+balance|balance\s+forward|start\s+balance|effective\s+date/i;
...
async function detectAndExtract(base64pdf) {
  ...
  console.log(`[textExtract:diag] cols colCount=${cols.colCount} debit=${cols.debit} credit=${cols.credit} balance=${cols.balance} rows=${rows.length}`);

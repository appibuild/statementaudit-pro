...
export default function App() {
  ...
  const [pdfDragActive,setPdfDragActive]= useState(false);// true while dragging — renders capture overlay
  ...
  // ── PDF split-pane drag ───────────────────────────────────────────────
  // The <object> element captures mouse events when the cursor enters it,
  // breaking drag. Fix: render a transparent overlay over the PDF during
  // drag so the document keeps receiving mousemove/mouseup.
  const startSplitDrag = useCallback(e => {
    ...
    const onMove = ev => {
      ...
      setPdfSplit(Math.min(70, Math.max(20, Math.round(pct))));
    };
    const onUp = () => {

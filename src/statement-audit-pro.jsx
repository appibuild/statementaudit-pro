            ...
            <div style={{display:'flex',alignItems:'center',gap:4,marginBottom:4}}>
              {renamingProjectId === activeProjectId
                ? <input autoFocus value={renameProjectVal}
                    ...
                    onBlur={commitRename}
                    style={{flex:1,background:C.card,border:`1px solid ${C.grn}`,borderRadius:5,padding:'4px 6px',color:C.t1,fontSize:11,outline:'none'}}/>
                : <select value={activeProjectId} onChange={e => setActiveProjectId(e.target.value)}
                    style={{flex:1,background:C.card,border:`1px solid ${C.bdrBrt}`,borderRadius:5,padding:'4px 6px',color:C.t1,fontSize:11,outline:'none',cursor:'pointer'}}>
                    {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
              }
              <button onClick={() => renameProject(activeProjectId)}
                style={{background:'none',border:`1px solid ${C.bdr}`,borderRadius:4,color:C.t3,fontSize:11,cursor:'pointer',padding:'3px 6px'}}>✎</button>
            </div>
            <button onClick={addProject}
              style={{width:'100%',background:'none',border:`1px dashed ${C.bdr}`,borderRadius:5,color:C.t3,fontSize:11,cursor:'pointer',padding:'4px'}}>+ New Project</button>
          </div>
              ...
              <div key={x.id} onClick={() => setActiveId(x.id)}
                ...
                {isA && projects.length > 1 && (
                  <select value={x.projectId||'default'} onClick={e => e.stopPropagation()}
                    onChange={e => moveStmt(x.id, e.target.value)}
                    style={{marginTop:5,width:'100%',background:C.surf,border:`1px solid ${C.bdr}`,borderRadius:4,padding:'3px 5px',color:C.t3,fontSize:10,outline:'none',cursor:'pointer'}}>
                    {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                )}
              </div>
